...
const dessertNamesByMbti: Record<string, { ko: string; en: string }> = {
  ...
  ENTP: { ko: "두바이 쫀득 쿠키", en: "Dubai Chewy Cookie" },
  ISTP: { ko: "퀸아망", en: "Kouign-Amann" },
  ...
  ISFP: { ko: "복숭아 데니쉬", en: "Peach Danish" },
...
export function DessertMarquee() {
  ...
  const [cards, setCards] = useState<typeof allResultImageCards | null>(null);
  ...
  useEffect(() => {
    ...
    applyShuffle();
    window.addEventListener("pageshow", applyShuffle);

    return () => {
      window.removeEventListener("pageshow", applyShuffle);
    };
  }, []);
  ...
  return (
    <section
      aria-label={t("메인 결과 카드 미리보기", "Sliding dessert card preview")}
